...
library(methods)    #R default
library(optparse)   #License: GPL >= 2
library(Matrix)     #License: GPL >= 2
library(irlba)      #License: GPL-3
...
ndocs <- read.table(opt$dfFile, header=FALSE, nrows=1, colClasses=c("numeric"))

ndocs <- ndocs[1, 1]

wordList <- readLines(opt$wordList)

numFeatures <- length(wordList)


sampleData <- read.table(opt$occurrMatrix, header=FALSE, colClasses=c("integer", "integer", "numeric"), col.names=c("row", "col", "val"), skip=1)
...
dfTable <- read.table(opt$dfFile, header=FALSE, skip=1, colClasses=c("numeric"))
...
writeMM(DissimsExport, file=opt$outFile)
